                         ...
                         TH2F* hcmap, TH2F* hcmap2)
{
	...
	for(int etaRing = 0; etaRing < nEtaRings; ++etaRing) {
		...
		//std::cout<<"check stat Gaussian par 2 "<<fgaus->GetParameter(2)<<std::endl;

		//std::cout<<etaMin <<" "<<fgaus->GetParameter(2)<<"  Error  "<<fgaus->GetParError(2)<<std::endl;
...
void InitializeDeadTT_EB_onlyW(std::vector<std::pair<int, int> >& TT_centre)
{
	...
	TT_centre.push_back(std::pair<int, int> (-43, 243));
        ...
        TT_centre.push_back(std::pair<int, int> (-18, 23));
        TT_centre.push_back(std::pair<int, int> (-8, 53));
        TT_centre.push_back(std::pair<int, int> (-28, 83));
        TT_centre.push_back(std::pair<int, int> (-28, 168));
        TT_centre.push_back(std::pair<int, int> (-53, 193));
        //TT_centre.push_back(std::pair<int, int> (-73, 218));
        TT_centre.push_back(std::pair<int, int> (-8, 223));
        TT_centre.push_back(std::pair<int, int> (-48, 248));
        TT_centre.push_back(std::pair<int, int> (-18, 313));
        //TT_centre.push_back(std::pair<int, int> (58, 48));
        //TT_centre.push_back(std::pair<int, int> (8, 113));
        //TT_centre.push_back(std::pair<int, int> (53, 173));
        TT_centre.push_back(std::pair<int, int> (18, 183));
        TT_centre.push_back(std::pair<int, int> (28, 258));
        TT_centre.push_back(std::pair<int, int> (8, 283));
...
int templIndexEB(const std::string& type, const float& eta, const float& charge, const float& R9)
{
	...
	if( type == "eta6" ) {
		if( (eta  < -1.)                  )  return 0;
		if( (eta >= -1.)  && (eta < -0.5) )  return 1;
		if( (eta >= -0.5) && (eta < 0.)   )  return 2;
		if( (eta >=  0.)  && (eta < 0.5)  )  return 3;
		if( (eta >=  0.5) && (eta < 1.)   )  return 4;
		if( (eta >=  1.)                  )  return 5;
	}
