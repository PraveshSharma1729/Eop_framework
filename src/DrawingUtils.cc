...
void DrawAvgICVsEtaGraph(TGraphErrors* g, const std::string& fileName, const std::string& fileType, const Color_t& color, const bool& isEB)
{
        if(isEB)
        {
		...
		g -> GetXaxis() -> SetRangeUser(-85,85);
        	g -> GetYaxis() -> SetRangeUser(0.900, 1.100);
		g -> GetYaxis() -> SetTitle("#LTIC#GT_{i#eta}");
