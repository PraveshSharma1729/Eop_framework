...
*/
  ...
  if( isEB == true )
  {
    ...
    if( evalStat )
    {
      ...
      if (h2_IC_raw_phiNorm_even[0]->GetEntries() == 0) std::cout << "h2_IC_raw_phiNorm_even[0] is empty." << std::endl;
      ...
      if (h2_IC_raw_phiNorm_odd[0]->GetEntries() == 0) std::cout << "h2_IC_raw_phiNorm_odd[0] is empty." << std::endl;
  ...
  if( isEB == true )
  {
	...
	if( etaBinCenter < 0. )
	{
	  ...
	  //	  cout<<"check1   x= "<<ibin<<" y= "<<jbin<< "IC "<<IC<<" fit "<<pol0_EBM->GetParameter(0)<<" ICPhifoldavg "<<ICPhiFoldAvg<< " final_value "<<IC * pol0_EBM->GetParameter(0) / ICPhiFoldAvg<<endl;
	}
	if( etaBinCenter > 0. )
	{
	  ...
	  //	  cout<<"check2   x= "<<ibin<<" y= "<<jbin<< "IC "<<IC<<" fit "<<pol0_EBM->GetParameter(0)<<" ICPhifoldavg "<<ICPhiFoldAvg<< " final_value "<<IC * pol0_EBM->GetParameter(0) / \
ICPhiFoldAvg<<endl;
